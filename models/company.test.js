...
describe('findAll', function() {
	...
	test('filter by min and max', async () => {
	...
	test('return empty list if filter returns nothing', async () => {
		let companies = await Company.findAll({ name: 'notgonnafindme' });
		expect(companies).toEqual([]);
	});
	test('throw bad request error if min >  max', async () => {
		try {
			await Company.findAll({ minEmployees: '1000', maxEmployees: '1' });
			fail();
		} catch (error) {
